...
WITH 
	...
	diagnosis_groups AS (
		...
		JOIN 
			...
			WHERE 
				c2.vocabulary_id = 'ICD9CM' AND c2.concept_class_id = '3-dig nonbill code'
			) ancestor
			ON c.concept_code LIKE ancestor.concept_code + '%'
		JOIN 
			@vocabulary_schema.concept_relationship cr 
			ON c.concept_id = cr.concept_id_1 AND cr.relationship_id = 'Maps to'
		JOIN 
			@vocabulary_schema.concept c3 
...
FROM (
	...
	JOIN 
		ricoveri
		ON ricoveri.person_id = co.person_id 
		AND co.condition_start_date >= (ricoveri.visit_start_date - 180) 
		AND co.condition_start_date < ricoveri.visit_start_date
) all_diag
GROUP BY all_diag.visit_occurrence_id, all_diag.person_id, all_diag.icd9_group, all_diag.icd9_group_name